...
class TransAllCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'langman:trans-all {key}';
     ...
     * @var string
     */
    protected $description = 'Add or update translations in all languages for the given key.';
    ...
    private function fillKey()
    {
        ...
        $values = $this->collectValue($languages);
    ...
    /**
     * Collect translation value from console via questions.
     *
     * @param $languages
     * @return array
     */
    private function collectValue($languages)
    {
        $values = [];

        $translation = $this->ask(sprintf('<fg=yellow>%s.%s</> translation', $this->fileName, $this->key));

        foreach ($languages as $languageKey) {
            $values[$languageKey] = $translation;
        }
